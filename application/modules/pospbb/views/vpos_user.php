...
$(document).ready(function() {
	oTable = $('#table1').dataTable({
		...
		"bInfo": false,
		"bProcessing": false,
		"sAjaxSource": "<?=active_module_url();?>pos_user/grid"
	});
	...
	$('#btn_tambah').click(function() {
		window.location = '<?=active_module_url();?>pos_user/add/';
	});

	$('#btn_edit').click(function() {
		if(mID) {
			window.location = '<?=active_module_url();?>pos_user/edit/'+mID;
		}else{
	...
	$('#btn_delete').click(function() {
		if(mID) {
			...
			if(hapus==true) {
				window.location = '<?=active_module_url();?>pos_user/delete/'+mID;
			};
		}else{
