...
class Apps extends CI_Controller {
	...
	function grid() {
		$i=0;
		if($query = $this->app_model->get_all()) {
	...
	function update_stat() {
		...
		if($id && $this->app_model->get_by_id($id)) {
			$data = array('disabled' => $val);
			$this->db->where('id', $id);
			$this->db->update('apps',$data);
		}
		
        if ($rows = $this->app_model->get_active_only()) {
	...
	public function add() {
		$data['current']     = 'pengaturan';
		$data['apps']    = $this->app_model->get_active_only();
		$data['faction']     = base_url('admin/apps/add');
		$data['dt']          = $this->fpost();
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$this->app_model->save($data);
			
			$this->session->set_flashdata('msg_success', 'Data telah disimpan');		
			redirect(base_url('admin/apps'));
		}
	...
	public function edit() {
		$data['current']   = 'pengaturan';
		$data['apps']    = $this->app_model->get_active_only();
		$data['faction']   = base_url('admin/apps/update');
			
		$id = $this->uri->segment(4);
		if($id && $get = $this->app_model->get_by_id($id)) {
			$data['dt']['id'] = $get->id;
	...
	public function update() {
		...
		if ($this->form_validation->run() == TRUE) {	
			...
			$this->app_model->update($this->input->post('id'), $data);
