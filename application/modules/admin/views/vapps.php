...
$(document).ready(function() {
	oTable = $('#table1').dataTable({
        ...
        "bFilter": true,
        "bAutoWidth": false,
		"sAjaxSource": "<?=base_url();?>admin/apps/grid"
	});
	...
	$('#btn_tambah').click(function() {
		window.location = '<?=base_url();?>admin/apps/add/';
	});

	$('#btn_edit').click(function() {
		if(mID) {
			window.location = '<?=base_url();?>admin/apps/edit/'+mID;
		}else{
	...
	$('#btn_delete').click(function() {
		if(mID) {
			...
			if(hapus==true) {
				window.location = '<?=base_url();?>admin/apps/delete/'+mID;
			};
		}else{
...
function update_stat(id, a) {
	var val = Number(a);
	$.ajax({
	  url: '<?php echo base_url()?>admin/apps/update_stat/' + id + '/' + val,
